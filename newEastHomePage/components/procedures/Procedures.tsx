...
interface IProceduresState {
  proceduresListItems: any;
}

export default class Proceudures extends React.Component<
  IProceduresProps,
  IProceduresState
> {
  public constructor(props: IProceduresProps, state: IProceduresState) {
    super(props);
    this.state = {
      proceduresListItems: null,
    };
  }
  ...
  public componentDidMount() {
    ...
    context.spHttpClient
      .get(
        `${context.pageContext.web.absoluteUrl}/_api/web/lists/getByTitle('PolicesProcedures')/items?$select=ID,FileRef,Created,Modified/FileRef,Editor/Title&$expand=Editor`,
      ...
      .then((listItems: any) => {
        ...
        console.log("procedures sortedItems", sortedItems);
        this.setState({
          proceduresListItems: sortedItems,
        });
        console.log(this.state.proceduresListItems, "procedures");
      });
  }

  /* private isFolder(fileUrl: string): boolean {
    return (
      fileUrl.search(
        /\.(jpe?g|png|bmp|gif|tiff|aiff|docx|doc|xlsx|pptx|ppt|pdf)$/i
      ) !== -1
    );
  } */

  public render(): React.ReactElement<IProceduresProps> {
    const { proceduresListItems } = this.state;
    const { context } = this.props;
    ...
    return (
      ...
      >
        {proceduresListItems?.length > 0 ? (
          proceduresListItems?.map((driveData: any) => {
            let fileIcon: any;
            switch (
